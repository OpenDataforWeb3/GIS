...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, rng):
    ...
    address = tokens[0][0]
    cand_indexes = np.arange(len(tokens))
    
    # Shuffle candidate indexes
    rng.shuffle(cand_indexes)
    
    # Create output_tokens as a NumPy array
    output_tokens = np.array(tokens, copy=True)
    
    num_to_predict = min(max_predictions_per_seq, max(1, int(len(tokens) * masked_lm_prob)))
    ...
    return (address, output_tokens.tolist(), masked_lm_positions, masked_lm_labels)
...
def main():
    vocab = FreqVocab()
    print("===========Load Sequence===========")
    file_path = '/home/paperspace/BERT4ETH_ODC/Model/inter_data/eoa2seq_bert4eth_exp.pkl'  # this is the data prepped for model - not lables

    with open(file_path, 'rb') as f:
        eoa2seq = pkl.load(f)

    with open("/home/paperspace/BERT4ETH_ODC/Loki_eoa2seq_2023-05-01.pkl", "rb") as f:  #this is labeled data
        my_eoa2seq = pkl.load(f)
        eoa2seq.update(my_eoa2seq)

    print("Before loading vocab:")
    print_memory_usage()
    ...
    print("Before generating samples:")
    print_memory_usage()
